...
var textureList = {
    'Golden' : 'golden.png',
    'Albedo01' : 'T_Bugatti_Chiron_MainBody_Albedo_01.png',
    'Albedo02' : 'T_Bugatti_Chiron_MainBody_Albedo_02.png',
    'AO' : 'T_Bugatti_Chiron_MainBody_AO.png'
}


function setTexture(mod, originalMat) {
    const texture = new THREE.TextureLoader().load('images/texture/' + 'golden.png');
    ...
    $('#texture_select').on('change', function () {
        let dropdown = document.getElementById('texture_select');
        textureRes = textureList[dropdown.value];
        const texture = new THREE.TextureLoader().load('images/texture/' + textureRes);
        texture.wrapS = THREE.RepeatWrapping;
        texture.wrapT = THREE.RepeatWrapping;
        texture.repeat.set(1, 1);
        material = new THREE.MeshPhongMaterial( { map: texture } );
        ...
        if (modelWithTextures || fbxLoaded || gltfLoaded) {
            textureCheck.checked ? mod.material = material : mod.material = originalMat;
        }
        else{
        textureCheck.checked ? mod.material = material : mod.material = materials.default_material;
        }
    });
    

    $('#texture_check').on('change', function () {
        if (modelWithTextures || fbxLoaded || gltfLoaded) {
            textureCheck.checked ? mod.material = material : mod.material = originalMat;
          }
          else{
